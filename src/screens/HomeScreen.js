...
export default function HomeScreen({ navigation }) {
  ...
  const carregarMateriais = useCallback(
    async (nome = "", idCategoria = null, idLocal = null) => {
      setLoading(true);
      try {
  ...
  const renderItem = ({ item }) => {
    ...
    const uriImagem = item.imagem
      ? item.imagem.startsWith("http")
        ? item.imagem
        : `${BASE_URL}/imagens/${encodeURIComponent(item.imagem)}`
      : null;
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() =>
          navigation.navigate("DetalhesMaterial", { material: item })
        }
        activeOpacity={0.82}
      >
        {uriImagem ? (
      ...
      <View style={styles.buscaContainer}>
        ...
        >
          <Ionicons
            ...
            size={18}
            color={filtrosAtivos ? "#1a73e8" : "#fff"}
          />
        ...
        <View style={styles.tagsContainer}>
          {categoriaSelecionada && (
            <TouchableOpacity
              ...
              onPress={() => {
                setCategoriaSelecionada(null);
                carregarMateriais(busca, null, localSelecionado?.id);
              }}
            >
          ...
          {localSelecionado && (
            <TouchableOpacity
              ...
              onPress={() => {
                setLocalSelecionado(null);
                carregarMateriais(busca, categoriaSelecionada?.id, null);
              }}
            >
            ...
            >
              {categorias.map((cat) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    categoriaSelecionada?.id === cat.id && styles.chipAtivo,
                  ]}
                  onPress={() =>
            ...
            >
              {locais.map((loc) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chip,
                    localSelecionado?.id === loc.id && styles.chipAtivo,
                  ]}
                  onPress={() =>
                    setLocalSelecionado(
                ...
                >
                  <Text
                    style={[
                      styles.chipText,
                      localSelecionado?.id === loc.id && styles.chipTextoAtivo,
                    ]}
                  >
                    {loc.nome}
                  </Text>

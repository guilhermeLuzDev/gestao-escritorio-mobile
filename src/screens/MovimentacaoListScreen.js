...
export default function MovimentacaoListScreen({ route }) {
    ...
    try {
      const response =
        materialId && materialId !== 0
          ? await api.get(`/movimentacao/patrimonio/${materialId}`)
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      <Text style={[styles.bold, { fontSize: 16, marginBottom: 8, color: '#202124' }]}>
        {item.patrimonio?.material?.nome || item.patrimonio?.nome || 'Material Desconhecido'} 
          ...
          !loading ? (
            <View style={styles.center}>
              ...
              <Text style={styles.vazioSubtitulo}>
                As transferências registradas aparecerão aqui.
              </Text>
            </View>
          ) : null
        }
